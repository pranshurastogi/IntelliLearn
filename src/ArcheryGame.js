...
const ArcheryGame = () => {
  ...
  const fireEnemyArrow = useCallback((enemyIndex) => {
    ...
    if (roundOver || !enemy || !enemyAi || enemyArrows.some(arrow => arrow.enemyIndex === enemyIndex)) {
  ...
  }, [roundOver, positions, enemyArrows]); // Dependencies
  ...
  const startNewRound = useCallback(() => {
    ...
    setPlayerArrow(null);
    setEnemyArrows([]); // Clear all enemy arrows
    ...
    if (canvasRef.current) {
        ...
        // Initialize AI state for all enemies for this round
        enemyAiRef.current = Array.from({ length: NUM_ENEMIES }, (_, i) => createInitialAiState(i));
  ...
  const learnFromMiss = useCallback((enemyIndex, missDistance) => {
    ...
    const missSeverity = Math.min(1, missDistance / (canvasRef.current?.width * 0.3 || 300)); // Normalized miss
    ...
    enemyAi.lastSpeed = Math.max(BASE_ENEMY_SPEED * 0.7, Math.min(BASE_ENEMY_SPEED * 1.8, enemyAi.lastSpeed));
  ...
  }, [roundOver]); // Dependencies


  // Handle a hit event (Player hits Enemy, or Enemy hits Player)
  const handleHit = useCallback((winner, targetInfo = null) => { // targetInfo could be enemyIndex if player hits enemy
    if (roundOver) return;
  ...
  const drawArrow = useCallback((x, y, angle) => {
    ...
    ctx.fillRect(-arrowLength * 0.6, -shaftWidth / 2, arrowLength * 0.8, shaftWidth); // ShiftedfillRect
    ...
    ctx.beginPath(); ctx.moveTo(fletchingBaseX + fletchingLength, -shaftWidth / 2); ctx.lineTo(fletchingBaseX, -fletchingWidth); ctx.lineTo(fletchingBaseX, -shaftWidth / 2); ctx.fill();
    ctx.beginPath(); ctx.moveTo(fletchingBaseX + fletchingLength, shaftWidth / 2); ctx.lineTo(fletchingBaseX, fletchingWidth); ctx.lineTo(fletchingBaseX, shaftWidth / 2); ctx.fill();
    ctx.restore();
  }, [ctx]); // Removed SCALE_FACTOR dependency
  ...
  const drawPlayerArcher = useCallback((x, y, angleDeg, powerPercent) => {
    if (!ctx) return;
    ctx.save();
    ctx.fillStyle = "blue"; ctx.strokeStyle = "blue"; ctx.lineWidth = 4 * SCALE_FACTOR;
    ...
    ctx.beginPath(); ctx.arc(x, headOffsetY, headRadius, 0, Math.PI * 2); ctx.fill(); // Head
    ctx.beginPath(); ctx.moveTo(x, bodyBottomY); ctx.lineTo(x - legSpread, groundY); ctx.stroke(); // Leg 1
    ctx.beginPath(); ctx.moveTo(x, bodyBottomY); ctx.lineTo(x + legSpread, groundY); ctx.stroke(); // Leg 2
    ctx.beginPath(); ctx.moveTo(x, bodyTopY); ctx.lineTo(x - 15 * SCALE_FACTOR, bodyTopY - 10 * SCALE_FACTOR); ctx.stroke(); // Back Arm

    const angleRad = (-angleDeg * Math.PI) / 180;
    const armEndX = x + armLength * Math.cos(angleRad);
    const armEndY = bodyTopY + armLength * Math.sin(angleRad);
    ctx.beginPath(); ctx.moveTo(x, bodyTopY); ctx.lineTo(armEndX, armEndY); ctx.stroke(); // Front Arm (Bow Arm)
    ...
    // Draw Bow
    ctx.lineWidth = 3 * SCALE_FACTOR; ctx.strokeStyle = "#8B4513"; // Brown bow
    ctx.beginPath(); ctx.arc(0, 0, bowRadius, Math.PI / 2, -Math.PI / 2, true); ctx.stroke(); // Bow arc

    // Draw Bow String
    const pullBack = (powerPercent / 100) * (15 * SCALE_FACTOR); // Scaled pullback
    ctx.lineWidth = 1 * SCALE_FACTOR; ctx.strokeStyle = "#A9A9A9"; // Grey string
    ctx.beginPath(); ctx.moveTo(0, -bowRadius); ctx.lineTo(-pullBack, 0); ctx.lineTo(0, bowRadius); ctx.stroke(); // Drawn string
    ...
    ctx.fillRect(-pullBack - visualArrowLength * 0.8, visualArrowYOffset - visualArrowShaftWidth / 2, visualArrowLength, visualArrowShaftWidth); // Shaft

    ctx.fillStyle = "#696969"; // Arrow head color (e.g., DimGray)
    ctx.beginPath();
    const headStartX = -pullBack - visualArrowLength * 0.8 + visualArrowLength; // Tip of the shaft
  ...
  }, [ctx]); // Removed angle, power, SCALE_FACTOR dependency
  ...
  const drawEnemyArcher = useCallback((x, y) => {
    if (!ctx) return;
    ctx.save();
    ctx.fillStyle = "red"; ctx.strokeStyle = "red"; ctx.lineWidth = 4 * SCALE_FACTOR;
    const headRadius = 12 * SCALE_FACTOR;
    const bodyTopOffsetY = -100 * SCALE_FACTOR;
    const bodyBottomOffsetY = -60 * SCALE_FACTOR;
    ...
    ctx.beginPath(); ctx.moveTo(x, bodyTopY); ctx.lineTo(frontArmX, frontArmY); ctx.stroke(); // Front arm (holding bow)
    ctx.beginPath(); ctx.moveTo(x, bodyTopY); ctx.lineTo(x + 15 * SCALE_FACTOR, bodyTopY - 10 * SCALE_FACTOR); ctx.stroke(); // Back arm (static)

    // Draw Static Bow relative to front arm end
    ctx.save();
    ctx.translate(frontArmX, frontArmY); // Move to bow anchor point
    ctx.lineWidth = 3 * SCALE_FACTOR; ctx.strokeStyle = "#8B4513"; // Brown bow
    ctx.beginPath(); ctx.arc(0, 0, bowRadius, Math.PI / 2, -Math.PI / 2, false); ctx.stroke(); // Semicircle facing player (left)
    ctx.lineWidth = 1 * SCALE_FACTOR; ctx.strokeStyle = "#A9A9A9"; // Grey string
    ctx.beginPath(); ctx.moveTo(0, -bowRadius); ctx.lineTo(0, bowRadius); ctx.stroke(); // Straight string (not drawn)
  ...
  const drawGame = useCallback(() => {
    ...
    ctx.fillStyle = "#228B22"; ctx.fillRect(0, canvas.height - groundHeight, canvas.width, groundHeight); // Ground
  ...
  }, [ ctx, positions, angle, power, playerArrow, enemyArrows,
  ...
  useEffect(() => {
    ...
    const animate = (timestamp) => {
      ...
      if (!roundOver) { // Only update physics and AI if round is active

          // --- Enemy Shooting Logic ---
          if (deltaTime > 0 && enemyAiRef.current.length === NUM_ENEMIES) { // Avoid running on first frame or if AI not ready
            enemyAiRef.current.forEach((aiState, index) => {
              ...
              if (aiState.nextShotDelay <= 0) {
                ...
                if (!hasArrow && positions.enemies[index]) { // Check position exists too
          ...
          if (playerArrow) {
              ...
              if (!canvas || newX < -outOfBoundsMargin || newX > canvas.width + outOfBoundsMargin || newY > groundY) {
                   setPlayerArrow(null); // Arrow out of bounds or hit ground
                    ...
                    if (hitEnemyIndex !== -1) {
                        ...
                        setPlayerArrow({ ...playerArrow, x: newX, y: newY, vx: newVx, vy: newVy });
                    }
              }
          }
          ...
          if (enemyArrows.length > 0) {
             const nextEnemyArrows = enemyArrows.map(arrow => {
                ...
                if (!canvas || newX < -outOfBoundsMargin || newX > canvas.width + outOfBoundsMargin || newY > groundY) {
                    ...
                    if (enemyAiRef.current[arrow.enemyIndex]) { // Ensure AI state exists
                ...
                return { ...arrow, x: newX, y: newY, vx: newVx, vy: newVy, minMissDistance: currentMinMiss };
             ...
             if (nextEnemyArrows.length !== enemyArrows.length) {
                setEnemyArrows(nextEnemyArrows);
                 ...
                 for(let i=0; i<nextEnemyArrows.length; i++){
                     if(nextEnemyArrows[i].x !== enemyArrows[i].x || nextEnemyArrows[i].y !== enemyArrows[i].y){
                         changed = true;
                         break;
                     }
                 }
  ...
  }, [ ctx, roundOver, playerArrow, enemyArrows, positions, angle, power,
       handleHit, learnFromMiss, drawGame, fireEnemyArrow, // Added fireEnemyArrow
  ...
  return (
    <div className="archery-game-container">
      ...
      <div className="controls-area">
          ...
          {roundOver && (
                <div className="round-over-message">Round Over! Starting next round...</div>
          )}
           {!ctx && (
